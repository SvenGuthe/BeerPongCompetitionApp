...
const App: React.FC = () => {
  ...
  const { registeredUser, authenticatedUser } = useSelector(
    (state: RootState) => {
      return {
    ...
    <Routes>
      <Route element={<Layout />}>
        ...
        <Route path="authentication">
          ...
          <Route
            path={registerHierarchy.relativeLabel}
            element={<Register />}
          />
        </Route>
        <Route path="confirm">
          {registeredUser && (
            <Route
              path={confirmWaitHierarchy.relativeLabel}
              element={<ConfirmWait />}
            />
          )}
          <Route
            path={confirmResultHierarchy.relativeLabel}
            element={<ConfirmResult />}
          />
        </Route>
        <Route path={teamHierarchy.relativeLabel}>
          {privileges?.find(
            (privilege) =>
              privilege.privilege === tSecurityPrivilege.ADMIN_TEAM_PRIVILEGE
          ) ? (
            <>
        ...
        <Route path={aclClassHierarchy.relativeLabel}>
          {privileges?.find(
            (privilege) =>
              privilege.privilege === tSecurityPrivilege.ADMIN_ACL_PRIVILEGE
          ) ? (
            <>
