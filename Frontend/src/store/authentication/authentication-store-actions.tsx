...
import { login, register, confirm, setAuthenticatedUser, setLoading } from "./authentication-store";
import { tRegister } from "../../types/authentication";
import { authenticatedUserRoute, confirmRoute, loginRoute, registerRoute } from "../../api-routes/authentication";

export const sendLoginRequest = (email: String, password: String) => {
    return async (dispatch: Dispatch<any>) => {
        console.log(`Send ${loginRoute} [POST] Request`);
        const sendRequest = async () => await axios.post(loginRoute, {
            username: email,
            password: password
        }).then((response) => {
            dispatch(login({
...
export const sendRegisterRequest = (registerFormData: tRegister) => {
    ...
    return async (dispatch: Dispatch<any>) => {
        const sendRequest = async () => await axios.post(registerRoute, {
            ...registerFormData
        }).then((response) => {
...
export const sendConfirmRequest = (token: string) => {
    ...
    return async (dispatch: Dispatch<any>) => {
        const sendRequest = async () => await axios.get(`${confirmRoute}?token=${token}`).then((response) => {
...
export const sendAuthenticationRequest = () => {
    ...
    return async (dispatch: Dispatch<any>) => {
        const sendRequest = async () => await axios.get(authenticatedUserRoute).then((response) => {
