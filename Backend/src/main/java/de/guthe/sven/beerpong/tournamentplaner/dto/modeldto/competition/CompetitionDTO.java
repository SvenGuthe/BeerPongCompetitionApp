...
public class CompetitionDTO extends ID {
	...
	public CompetitionDTO(Long id, String competitionName, Timestamp competitionStartTimestamp, Integer minTeams,
			Integer maxTeams, Double fee, Timestamp registrationStart, Timestamp registrationEnd, String setOfRules,
			Timestamp creationTime, Collection<CompetitionStatusDTO> competitionStatus,
			Collection<CompetitionTeamDTO> competitionTeams, Collection<CompetitionAdminDTO> competitionAdmins) {
		super(id);
	...
	public CompetitionDTO(Competition competition) {
		...
		this.competitionStatus = competition.getCompetitionStatusHistories().stream().map(CompetitionStatusDTO::new)
				.collect(Collectors.toList());
		this.competitionTeams = competition.getCompetitionTeams().stream().map(CompetitionTeamDTO::new)
				.collect(Collectors.toList());
		this.competitionAdmins = competition.getCompetitionAdmins().stream().map(CompetitionAdminDTO::new)
				.collect(Collectors.toList());
	}
