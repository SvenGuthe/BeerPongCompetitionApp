...
public interface CompetitionPlayerStatusRepository extends JpaRepository<CompetitionPlayerStatus, Long> {
    ...
    @Query(value = "SELECT * FROM CompetitionPlayerStatus cps WHERE LOWER(cps.competitionplayerstatusdescription) LIKE CONCAT(LOWER(?1), '%') OR cps.competitionplayerstatusid LIKE CONCAT(?1, '%')",
            countQuery = "SELECT count(*) FROM CompetitionPlayerStatus cps WHERE LOWER(cps.competitionplayerstatusdescription) LIKE CONCAT(LOWER(?1), '%') OR cps.competitionplayerstatusid LIKE CONCAT(?1, '%')",
            nativeQuery = true)
